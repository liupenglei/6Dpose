    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            if len(layers) == 1:
                ...
                print('%5d %-6s %d %d %d' % (ind, 'route', layers[0], layers[1], layers[2]))
                prev_width = out_widths[layers[0]]
                prev_height = out_heights[layers[0]]
                assert (prev_width == out_widths[layers[1]] == out_widths[layers[2]])
                assert (prev_height == out_heights[layers[1]] == out_heights[layers[2]])
            ...
            print('%5d  %-6s      x %d    %3d x %3d x %4d  ->  %3d x %3d x%4d' % (
            ...
            print('cfg unknown type %s' % (block['type']))
