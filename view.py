        ...
        for box_pr, target in zip([all_boxes], [target[0]]):
            ...
            for k in range(num_gts):
                ...
                if visualize:
                    ...
                    #plt.imshow(scipy.misc.np.array(Image.fromarray(arr).resize(img, (im_height, im_width))))
                    plt.imshow(scipy.misc.imresize(img, (im_height, im_width)))
                    # Projections

                    if all_boxes[18] > 0.5:
                        for edge in edges_corners:
                            plt.plot(proj_corners_pr[edge, 0], proj_corners_pr[edge, 1], color='r', linewidth=3.0)
...
weightfile = 'backup/duck/model.weights'
